...
from .models import Reply, Post, UserRegisterCode
from django.conf import settings


@receiver(post_save, sender=Reply)
def notify_add_reply (sender, instance, created, **kwargs):
    if created:
        send_mail( 
...
@receiver(post_save, sender=Reply)
def notify_apply_reply (sender, instance, created, **kwargs):
    if not created:
        send_mail( 
...
@receiver(post_save, sender=UserRegisterCode)
def notify_add_code (sender, instance, created, **kwargs):
    if created:
        send_mail( 
            subject=f'Код для завершения регистрации на сайте',
            message=f'Для завершения регистрации на странице http://127.0.0.1:8000/posts/sign/code введите следующий код:\n\n  {instance.code}',
            from_email=settings.DEFAULT_FROM_EMAIL,
            recipient_list=[instance.userCode.email]
        ) 
